...
class Transport extends EventEmitter {
    constructor(pipe, maxMessageSize) {
        ...
        const checkBuffer = () => {
            ...
            if (bufferPointer > headerLength && bufferPointer >= totalMsgLength) {
                const message = BSON.deserialize(buffer.slice(headerLength, headerLength + bodyLength), { promoteBuffers: true });
    ...
    send(message) {
        let serializedMessage = BSON.serialize(message, false, true, false);
