...
test('should be disabled by default', async (t) => {
    // given
    const workerNodes = new WorkerNodes(fixture('harmful-module'), { maxWorkers: 1 });
...
test('should give up after max retries limit has been reached', async (t) => {
    // given
    const workerNodes = new WorkerNodes(fixture('harmful-module'), { maxWorkers: 1, taskMaxRetries: 3 });
...
test('should catch thrown exceptions', async (t) => {
    // given
    const workerNodes = new WorkerNodes(fixture('harmful-module'), { maxWorkers: 1, taskMaxRetries: 0 });
...
test('should put through number of retries', async (t) => {
    // given
    const workerNodes = new WorkerNodes(fixture('harmful-module'), { maxWorkers: 1, taskMaxRetries: Infinity });
    await wait(5000);
